...
**Read gateways (8):** `hub_read_apps_code` (11), `hub_read_dashboards` (2), `hub_read_devices` (6), `hub_read_diagnostics` (8), `hub_read_files` (2), `hub_read_rooms` (2), `hub_read_rules` (6), `hub_read_variables` (3)
...
**Protected apps.** Select installed apps under **Protected apps** in the MCP app UI to block generic app, native-rule, and Dashboard mutations (Easy and legacy) targeting them. Protection applies in flat and gateway mode, with Developer Mode either on or off; reads remain available. The MCP instance is preselected for new installs and upgrades. Subsequent updates preserve your selections, including an empty list. Change protection in the Hubitat UI and click **Done** to apply it. Dedicated Developer Mode settings and package maintenance retain their existing gates, but `hub_update_mcp_settings` cannot change the list.
